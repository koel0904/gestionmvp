...
export default function EditTareaModal({ onClose, onSubmit, localId, initialTask }) {
  const [form, setForm] = useState({
    title: initialTask?.title || "",
    description: initialTask?.description || "",
    deadline: initialTask?.deadline ? format(new Date(initialTask.deadline), "yyyy-MM-dd'T'HH:mm") : "",
    isPublic: initialTask?.isPublic || false,
    assignedUserIds: initialTask?.assignedUsers ? initialTask.assignedUsers.map(u => u.id) : [],
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-lg glass-panel rounded-3xl p-6 shadow-2xl overflow-y-auto max-h-[90vh] custom-scrollbar">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <button 
            type="submit" 
            className="w-full mt-4 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-bold py-3 px-4 rounded-xl shadow-lg transition-all"
          >
            Guardar Cambios
          </button>
