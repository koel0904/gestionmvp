...
export default function ProfileTab() {
  ...
  // Sync form when global user info updates (e.g. after refresh/save)
  useEffect(() => {
    if (user) {
      setForm({
        name: user.name || "",
  ...
  const handleSave = async (e) => {
    ...
    try {
      await updateProfile({
        name: form.name,
        email: form.email,
        ...(user?.type !== "owner" && { position: form.position }), // Solo mandamos position si no somos dueños
      });
      setSaved(true);
      ...
      <Section title="Personal Information">
        <form onSubmit={handleSave} className="flex flex-col gap-4">
          ...
          {isOwner ? (
            ...
            <FieldRow label="Position" icon="work">
              <input
                type="text"
                name="position"
                value={form.position}
                onChange={handleChange}
                placeholder="e.g. Chef, Cocinero, Gerente..."
                className="glass-input rounded-xl px-4 py-3 text-sm text-white placeholder-white/20 outline-none w-full"
              />
            </FieldRow>
          )}
